 ...
 */
public class Pshufhw extends CalvisAnimation {
    ...
    public void animate(ScrollPane scrollPane) {
        ...
        Rectangle srcRec = this.createRectangle(src, 245, 165);
        ...
        Text zeroExtended = new Text(this.createLabelText(des).getText() + "[0 to 15] <- Retain: " + desStr.substring(16, 32));
        Text desFirstWord = new Text(this.createLabelText(des).getText() + "[16 to 19] <- " + desStr.substring(12, 16));
        Text desSecondWord = new Text(this.createLabelText(des).getText() + "[20 to 23 <- " + desStr.substring(8, 12));
        Text desThirdWord = new Text(this.createLabelText(des).getText() + "[24 to271] <- " + desStr.substring(4, 8));
        Text desFourthWord = new Text(this.createLabelText(des).getText() + "[28 to 31] <- " + desStr.substring(0, 4));

        parent.addAll(desFirstWord, desSecondWord, desThirdWord, desFourthWord, extractList, zeroExtended);
        ...
        zeroExtended.setX(posX + (srcRec.getLayoutBounds().getWidth() / 2 - zeroExtended.getLayoutBounds().getWidth() * 0.5));
        zeroExtended.setY(posY + 80 + desRec.getHeight() + 45);

        desFirstWord.setX(posX + (srcRec.getLayoutBounds().getWidth() / 2 - desFirstWord.getLayoutBounds().getWidth() * 0.75));
        desFirstWord.setY(posY + 100 + desRec.getHeight() + 45);

        desSecondWord.setX(posX + (srcRec.getLayoutBounds().getWidth() / 2 - desSecondWord.getLayoutBounds().getWidth() * 0.75));
        desSecondWord.setY(posY + 120 + desRec.getHeight() + 45);

        desThirdWord.setX(posX + (srcRec.getLayoutBounds().getWidth() / 2 - desThirdWord.getLayoutBounds().getWidth() * 0.75));
        desThirdWord.setY(posY + 140 + desRec.getHeight() + 45);

        desFourthWord.setX(posX + (srcRec.getLayoutBounds().getWidth() / 2 - desFourthWord.getLayoutBounds().getWidth() * 0.75));
        desFourthWord.setY(posY + 160 + desRec.getHeight() + 45);

...
//        for(int x = 0; x < (size/2) / 2; x = x + operationSize){
            dividedStrDes += desStr;
            dividedStrSrc += srcStr;
            dividedStrRes += resultStr;
//        }
        ...
        Rectangle desRec = this.createRectangle(des, 200, 60);
        Rectangle srcRec = this.createRectangle(src, 200, 60);
        Rectangle resRec = this.createRectangle(des, 200, 60);
        ...
        Text flowIndication = new Text();
        String indicationStr = "";
        if(registers.getMxscr().getOverflowFlag().equals("1") && finder.getMxscr().getOverflowFlag().equals("0")){
            indicationStr = "Resulting value is greater than 2^64. Overflow Exception is set.";
        }
        else if(registers.getMxscr().getUnderflowFlag().equals("1") && finder.getMxscr().getUnderflowFlag().equals("0")){
            indicationStr = "Resulting value is less than 2^64 - 1. Underflow Exception is set.";
        }
    }
}

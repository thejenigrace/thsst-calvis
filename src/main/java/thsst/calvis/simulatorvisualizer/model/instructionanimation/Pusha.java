...
import thsst.calvis.simulatorvisualizer.model.PushModel;
 ...
 */
public class Pusha extends CalvisAnimation {
...
//        String desVal = timeFunc.getValue(des, operandSize);
        ...
        for(int x = 0; x < registerList.size(); x++){
            String multiplier = Integer.toString(2 + 2 * x);
            BigInteger result = new BigInteger(currentSp, 16).subtract(new BigInteger(multiplier, 16));
            spList.add(c.hexZeroExtend(result.toString(16).toUpperCase(), 4));
        }
        ...
        for(int x = 0; x < pushModels.size(); x++){
            ...
            pushLabelArr.add(timeFunc.generateText(new Text("PUSHED VALUE"), 14, "#b31b1b", FontWeight.BOLD, "Elephant"));
            espValArr.add(timeFunc.generateText(new Text(pushModels.get(x).getSP()), 13, "#3d2b1f", FontWeight.NORMAL, "Elephant"));
            insLabelArr.add(timeFunc.generateText(new Text("Value will be pushed to =>"), 15, "#b31b1b", FontWeight.BOLD, "Elephant"));
            parent.addAll(insLabelArr.get(x), desRecArr.get(x), desValueArr.get(x), desLabelArr.get(x), stackRecArr.get(x), espValArr.get(x), stackLabelArr.get(x), desValueMovingArr.get(x), espLabelArr.get(x), pushLabelArr.get(x));
            //        Timeline animations
            desValueArr.get(x).setX(desRecArr.get(x).getLayoutBounds().getWidth() / 2 - desValueArr.get(x).getLayoutBounds().getWidth() / 2 + 20);
            desValueArr.get(x).setY(80 + (140 * x));

            desValueMovingArr.get(x).setVisible(false);

            timeFunc.addTimeline(desRecArr.get(x).getLayoutBounds().getWidth() / 2 - desValueMovingArr.get(x).getLayoutBounds().getWidth() / 2 + 20, 80 + (140 * x), 0, desValueMovingArr.get(x));
            timeFunc.addTimeline(stackRecArr.get(x).getLayoutBounds().getWidth() / 2 - desValueMovingArr.get(x).getLayoutBounds().getWidth() / 2 + 400, 100 + (140 * x), 3000, desValueMovingArr.get(x));
        ...
        desValueArr.get(4).setText(finder.getRegister("ESP").substring(4, 8));
        desValueMovingArr.get(4).setText(finder.getRegister("ESP").substring(4, 8));

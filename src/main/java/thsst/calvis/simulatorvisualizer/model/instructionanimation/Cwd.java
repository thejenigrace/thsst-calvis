 ...
 */
public class Cwd extends CalvisAnimation {
...
//        }
        ...
        Rectangle desRectangle = this.createRectangle(tokenDX, width, height);
        Rectangle srcRectangle = this.createRectangle(tokenAX, width, height);

        Text text = new Text("The sign of register AX is extended to register DX.\n" +
                "Affected flags: none");
        ...
        if ( desRectangle != null && srcRectangle != null ) {
            ...
            int desSize = 0;
            if ( tokenDX.getType() == Token.REG )
                desSize = registers.getBitSize(tokenDX);
            else if ( tokenDX.getType() == Token.MEM && tokenAX.getType() == Token.REG )
                desSize = registers.getBitSize(tokenAX);
            else
                desSize = memory.getBitSize(tokenDX);

            Text desLabelText = new Text("DX");
            Text desValueText = new Text(registers.get(tokenDX.getValue() + ""));

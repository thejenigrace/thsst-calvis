	...
	public ErrorMessageListWithSize checkFileNotFoundMessage(ArrayList<String> ConfigurationFilePaths){
		...
		for( int x = 0; x < ConfigurationFilePaths.size(); x++ ){
			...
			if( ! file.exists() ){
				switch(x){
					case 0:
						ErrorMessagesList.add(new ErrorMessage(Types.missingConfigFile, HandleConfigFunctions.generateArrayListString(file.getAbsolutePath()), ""));
						break;
					case 1:
						ErrorMessagesList.add(new ErrorMessage(Types.missingRegisterFile, HandleConfigFunctions.generateArrayListString(file.getAbsolutePath()), ""));
						break;
					case 2:
						ErrorMessagesList.add(new ErrorMessage(Types.missingInstructionFile, HandleConfigFunctions.generateArrayListString(file.getAbsolutePath()), ""));
						break;
				}
			}
		}
	...
	public void showErrorList(ErrorLogger errorLogger){
		...
		for( int x = 0; x < errorList.size(); x++ ){
			...
			for( int y = 0; y < errorList.get(x).getSizeofErrorMessages(); y++ ){
				Error = Error + new ErrorType(errorList.get(x).getListOfErrorMessages().get(y).getType()).generateType(errorList.get(x).getListOfErrorMessages().get(y).getVariables(), errorList.get(x).getListOfErrorMessages().get(y).getErrorLine());

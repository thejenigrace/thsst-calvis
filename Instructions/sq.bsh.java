execute(des, src, registers, memory) {
	if ( des.isMemory() ) {
		if ( src.isRegister() ) {
			String[] arrReturn = getRegisterProper(src.getValue(), registers, memory);
			int desRegSize = registers.getBitSize(src);
			String x = registers.get(arrReturn[0]);
			memory.write(des, x, desRegSize);
